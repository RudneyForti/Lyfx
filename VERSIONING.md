...
| `1.14.1` | PATCH | Lote de manutenção pós-auditoria sistêmica. Segurança: OAuth exige `email_verified` do Google e linking Microsoft restrito ao claim `email` (preferred_username fora da equação); timeout `AbortSignal` em todos os 8 fetch externos (Google Maps/Directions, userinfo, BrasilAPI, Turnstile); sessão admin do Studio com cookie HMAC-assinado e expiração server-side (era valor estático). Arquitetura: `app/studio/actions.ts` (910 linhas) fatiado em 7 módulos por responsabilidade com barrel de compatibilidade; `execSync` isolado em `system-info.ts` com timeout 3s. Performance: 3 índices na `Transaction` (userId+date, userId+category+date, installmentGroupId); `syncDangerAlerts` com throttle 1×/5min fire-and-forget (~7 queries a menos por navegação); badge inline no layout (1 consulta de sessão a menos); `getMonthlyTrend` via groupBy no banco; `StudioLoginForm` em chunk próprio; `lastSeenAt` com throttle 60s; seed único do AppConfig; novo `lib/throttle-gate.ts`. Limpeza: deps órfãs SQLite removidas (better-sqlite3, adapter, types) e artefatos `dev.db` deletados. |
| `1.14.0` | MINOR | CS-20: Quadro Kanban de Change Specs no Studio — board estilo Trello com 4 colunas (Backlog, Em andamento, Bloqueado, Concluídas), drag-and-drop HTML5, modal de detalhes editável (título, descrição, labels, versão, commit hash, completedAt), ordenação por mais nova/antiga por coluna, persistência em `docs/cs-board.json` (sobrevive reset do banco), pré-populado com CS-01→CS-38. CS-37a: 2FA TOTP completo — setup com QR Code (otplib + qrcode), verificação de 6 dígitos, 8 códigos de backup (XXXX-XXXX-XXXX, bcrypt), cookie HMAC lyfx_2fa (5 min) entre os passos de login, desativação e regeneração de backups via perfil, 4 novos eventos no Audit Log (2fa.enabled/disabled/failed/backup_used). Tela de erro redesenhada com `lim f(t) = ∅` em display font e tela 404 com logo corrigido. |
| `1.13.0` | MINOR | CS-34: Sessões com estado no banco — invalidação server-side, view de sessões ativas no perfil, revogação individual e em lote, cookie HMAC com sessionId+userId. CS-35: Audit Log de segurança — registro de login/logout/senha/sessão, histórico no perfil do usuário e aba Segurança no Studio com filtros por usuário e evento. CS-36: OAuth Google + Microsoft — login social via Arctic (PKCE), modelo OAuthAccount, find-or-create por providerUserId/email, botões adaptativos (ativos/desabilitados conforme .env), erros via cookie flash (URL limpa). |
...
| `1.15.4` | PATCH | Completa o fix da v1.15.3: o `COPY` dos arquivos falhava no build porque o `.dockerignore` excluía `docs/` e `*.md`, então `docs/cs-board.json` e `DOCUMENTATION.md` nem entravam no contexto de build (deploy da v1.15.3 quebrou no `docker build`). Adicionadas exceções `!docs/cs-board.json` e `!DOCUMENTATION.md`. Contexto de build validado antes de taggear. |
| `1.15.3` | PATCH | Fix de produção: `/studio` retornava 500 (`ENOENT /app/docs/cs-board.json`). O build standalone do Next.js não inclui arquivos lidos via `fs.readFile` com caminho computado, então `docs/cs-board.json` (Kanban) não entrava na imagem. Dockerfile agora copia explicitamente `docs/cs-board.json` e `DOCUMENTATION.md` pro runner; `getKanbanBoard` blindado contra ENOENT (retorna board vazio em vez de crashar). Primeiro bug de produção pego pela esteira de deploy automático. |
| `1.15.2` | PATCH | Fix do deploy-on-tag (v1.15.1): os passos do `deploy-prod.yml` usavam `shell: bash`, que no runner Windows resolve para o bash do WSL (`system32\bash.exe`) e falha sem distro WSL instalada. Reescritos em `shell: powershell` (Windows PowerShell 5.1, sempre presente) com checagem de `$LASTEXITCODE` por comando. Primeira release a deployar de fato pela esteira automática. |
| `1.15.1` | PATCH | CI/CD: deploy automático da produção (porta 4000) ao empurrar uma tag `v*`. Workflow `.github/workflows/deploy-prod.yml` em runner self-hosted (label `lyfx-prod`) na máquina host — checkout da tag no worktree de produção, rebuild da imagem e recriação dos containers (o serviço `migrate` aplica schema). Produção passa a rastrear a última tag de release, não o `main`-HEAD. Mantém o portão humano (taggear é decisão humana) e automatiza rebuild + restart + migrate. E7 passo 10 atualizado em `AGENTS.md` / `GIT-WORKFLOW.md`. |
| `1.15.0` | MINOR | Primeiro lote sob GitHub Flow. **Kanban v2 (CS-59):** checklists com barra de progresso, datas de início/prazo com chip de vencimento, comentários com log de atividade automático de movimentação, agrupamento por release na coluna Concluídas e filtro por label; schema v2 do board com migração v1→v2 idempotente (`lib/kanban.ts`). **Studio:** roteamento ortogonal A* das linhas do ERD (comportamento Power BI — nunca cruzam as caixas nem saem do canvas, recalculam ao arrastar; `lib/erd-router.ts`). **Processo/infra:** migração para GitHub Flow (branch única `main` + PRs, CODEOWNERS, template de PR); retrofit de testes de 0 → 90+ (44 unit, 41 integração, 5 E2E) com Vitest + Playwright e mock de APIs externas; lint restaurado como gate bloqueante; pipeline CI de 3 jobs (typecheck/lint/build, testes, E2E). **Governança:** git tags como fonte única de verdade de versão, gatilho de release proativo e reconciliação board×git (`AGENTS.md` / `GIT-WORKFLOW.md`); board reconciliado (8 cards fantasma `1.15.0` → `1.14.1`). **i18n:** todos os comentários de código traduzidos para inglês. **Docs:** versão bilíngue PT/EN de README, VERSIONING, FEATURES e DOCUMENTATION, com auditoria de fatos desatualizados contra o código real. **Fix:** datas de formulário parseadas como horário local (`lib/dates.ts`) — corrige transação do dia 1 caindo no mês anterior em UTC-3. **Planejamento:** cards CS-49→CS-59 adicionados ao board. |
