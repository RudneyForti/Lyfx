...
| `1.11.1` | PATCH | CS-23: containerização Docker. Dockerfile multi-stage (deps → builder → runner standalone). `output: "standalone"` em next.config.ts. docker-compose.yml (prod, porta 4000) + docker-compose.dev.yml (dev, porta 3000). Scripts npm: docker:build, docker:up, docker:down, docker:logs, docker:dev. .dockerignore + .env.docker.example com instrução host.docker.internal. Fix Prisma standalone: .prisma/client copiado explicitamente no stage runner. |
